...
def build_cfg(args):
    ...
    pretrain_cfg = Config(TEMPLATE_MODEL_CFG)
    pretrain_cfg.parse(args.pretrain_model_cfg)
    return data_cfg, pretrain_cfg
...
def train_step(img, label, label_length):
    ...
    gradients = tape.gradient(loss, task_net.trainable_variables)
    optimizer.apply_gradients(zip(gradients, task_net.trainable_variables))
...
parser.add_argument('-p', '--pretrain_model_cfg', type=str, help='pretrained model configuration', required=True)
parser.add_argument('-m', '--model_cfg', type=str, help='finetune model configuration', required=True)
args = parser.parse_args()
cfg, pretrain_cfg = build_cfg(args)

extractor_net = ExtractorNetwork(cfg)
task_net = TaskSpecificNetwork(cfg)

extractor_net.load_weights(pretrain_cfg.model_dir + '.extract')
...
print(f'Saved Weights from Epoch {best_valid_epoch}')